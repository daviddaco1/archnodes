...
- **Canvas visual** (React Flow) para ubicar, conectar y organizar nodos dentro de contenedores/boundaries, con panel de propiedades, paleta de nodos, panel de validación y pestaña de resumen del proyecto. Las relaciones por campo de referencia (ej. `table.dbId`) se muestran como puertos propios y etiquetados —estilo Unity Shader Graph— en vez de un único conector genérico, para que un nodo con varias relaciones no termine siendo un solo punto enredado.
- **Esquema de nodos tipado** que cubre tanto conceptos de backend (`domain`, `subdomain`, `route`, `endpoint`, `operation`, `middleware`, `service`, `model`, `table`, `db`, `orm`, `repository`, `tool`, `queue`, `externalApi`, `scheduler`, `errorHandler`, `envConfig`, `websocket`, `websocketEvent`, `websocketEmit`, `email`, `redisKey`) como de frontend (`page`, `layout`, `component`, `form`, `stateStore`, `apiCall`, `hook`, `navigationRouter`, `guard`, `modalDialog`, `themeToken`, `asset`), más nodos estructurales `container`/`boundary`/`note`.
- **Endpoints con varios métodos**: un nodo `endpoint` lista todos los métodos HTTP que soporta y guarda lo compartido (headers, config de caché, si es pública y sus métodos de seguridad); un nodo `operation` opcional por método guarda el query/params/body y outputs propios de ese método, para no tener que crear un endpoint distinto por cada GET/POST/etc. de la misma ruta.
- **Reglas de validación**: restricciones de jerarquía, campos requeridos, integridad de campos de referencia, consistencia de método por operación (debe estar entre los métodos declarados del endpoint, sin duplicados entre hermanas), y aristas especiales (por ejemplo, relaciones de invalidación de caché `invalidates`).
